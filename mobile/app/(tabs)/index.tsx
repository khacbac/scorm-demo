...
export default function ScormViewerScreen() {
  ...
  const handleMessage = useCallback(async (event: WebViewMessageEvent) => {
    try {
      ...
      if (message.type === 'SCORM_STATE') {
        ...
        await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(message.payload.cmi))
      }
    } catch {
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.viewerSection, { borderBottomColor: theme.icon + '22' }]}>
        <WebView
          ref={webViewRef}
      ...
      <ScrollView style={styles.inspector} contentContainerStyle={styles.inspectorContent}>
        ...
        {!scormState && (
          <ThemedText style={styles.notice}>Waiting for SCORM API state…</ThemedText>
        )}
        {scormState && (
          <>
            <View style={[styles.card, { borderColor: theme.icon + '22' }]}>
              ...
              <ThemedText style={styles.statusLine}>
                Status: <ThemedText style={styles.code}>{scormState.status}</ThemedText>
              </ThemedText>
              ...
              {scormState.logs.length === 0 ? (
                ...
                scormState.logs
                  ...
                  .map((log, index) => (
                    <View
                      key={`${log.time}-${index}`}
                      style={[styles.logItem, { borderTopColor: theme.icon + '22' }]}>
